program pair_vortices
  ...
  call vortex%init(Nvor,R_rnk,1.2_rp,0.02_rp)
  vortex%N          = Nvor
  vortex%has_vortex = .TRUE.
  vortex%Gama(       1:Nvor/2) =  1._rp/(Nvor/2)
  vortex%Gama(Nvor/2+1:Nvor  ) = -1._rp/(Nvor/2)
  ...
  DEALLOCATE(rth)
  vortex%r(1,:Nvor/2)   = vortex%r(1,1:Nvor/2)  + 0.05_rp
  vortex%r(2,:Nvor/2)   = vortex%r(2,1:Nvor/2)  + 0.05_rp
  vortex%r(1,Nvor/2+1:) = vortex%r(1,Nvor/2+1:) - 0.05_rp
  vortex%r(2,Nvor/2+1:) = vortex%r(2,Nvor/2+1:) - 0.05_rp
